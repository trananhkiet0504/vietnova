...
const menuItems = ['Dashboard', 'Residents', 'Units', 'Maintenance', 'Billing', 'Announcements'];
      ...
      <nav className="flex items-center gap-6 text-sm font-medium text-gray-700">
        {menuItems.map((item) => (
          <a
            ...
            className={`hover:text-blue-600 transition-colors ${item === 'Dashboard' ? 'text-black font-semibold' : ''}`}
          >
            {item}
          </a>
        ))}
      ...
      <div className="flex items-center gap-4">
        ...
        <Dropdown overlay={userMenu} placement="bottomRight">
          <div className="flex items-center cursor-pointer space-x-2">
            ...
            <span className="text-sm font-medium text-gray-800 hidden sm:inline">Admin</span>
          </div>
        </Dropdown>
      </div>
    </header>
